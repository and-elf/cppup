...
extern "C" BuildConfiguration configure()
{
  ...
  config.packages.push_back(Package::from_tar(
      "catch2", "https://github.com/catchorg/Catch2/archive/refs/tags/v3.4.0.tar.gz"));
  ...
  config.compile_flags = {Flag{"-std=c++23"}, Flag{"-Wall"}, Flag{"-Wextra"}, Flag{"-O2"}};

  config.binaries = {Binary{"package_demo", {"src/main.cpp"}}};
  ...
  if (get_env("ENABLE_TESTING").has_value())
  {
    ...
    config.tests = {Test{"package_demo_tests", {"tests/*.cpp"}}};
  }
...
extern "C" BuildConfiguration configure_platform_specific()
{
  ...
  // Common packages
  config.packages.push_back(Package::from_git("spdlog", "https://github.com/gabime/spdlog.git"));
  ...
  when_windows(
      [&]()
      {
        ...
        config.packages.push_back(Package::from_git("wil", "https://github.com/microsoft/wil.git"));
