...
extern "C" BuildConfiguration configure() {
    ...
    config.packages.push_back(
        header_only("nlohmann_json", "https://github.com/nlohmann/json.git")
    );
    ...
    config.binaries = {
        Binary{"modular_demo", {"src/main.cpp"}}
    };
...
extern "C" BuildConfiguration configure_conditional() {
    ...
    if (PackageFactory::is_build_system_available("cmake")) {
        config.packages.push_back(
            from_git("opencv", "https://github.com/opencv/opencv.git", "4.8.0")
        );
    }
    ...
    if (PackageFactory::is_build_system_available("make")) {
        config.packages.push_back(
            from_git("zlib", "https://github.com/madler/zlib.git", "v1.2.13")
        );
    }
    
    // Always available - cppup build system
    config.packages.push_back(
        from_git("my_cppup_lib", "https://github.com/user/my_cppup_lib.git")
    );
