...
extern "C" BuildConfiguration complex_real_world() {
    ...
    platform::add_platform_packages(config,
        {Package{"winsock2"}, Package{"bcrypt"}},           // Windows
    ...
    auto testing = test_profile("catch2", {
        Flag{"-coverage"},
        Flag{"-fprofile-arcs"},
        Flag{"-ftest-coverage"}
    });
    ...
    // Custom build steps
    config.build_steps = {
        BuildStep("generate_docs", []() {
            std::cout << "Generating documentation...\n";
        }),
        
        BuildStep("run_linter", []() {
            std::cout << "Running code linter...\n";
        }),
        
        BuildStep("package", []() {
            std::cout << "Creating distribution package...\n";
