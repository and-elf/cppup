...
class MakePackage
{
 ...
 private:
  ...
  [[nodiscard]] static bool has_makefile(const std::filesystem::path& source_path);
  [[nodiscard]] std::expected<void, std::string> execute_make(
      const std::filesystem::path& source_path) const;
  void                      setup_build_flags(const std::filesystem::path& source_path) const;
  [[nodiscard]] std::string get_make_command() const;
};
