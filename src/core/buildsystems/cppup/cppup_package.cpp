...
std::expected<void, std::string> CppupPackage::build(const std::filesystem::path& source_path) const {
    ...
    if (!has_build_file(actual_source_path)) {
        return std::unexpected("No build.cpp found in source directory: " + actual_source_path.string());
    }
...
std::expected<void, std::string> CppupPackage::execute_cppup_build(const std::filesystem::path& source_path) const {
    if (!command_executor_) {
...
void CppupPackage::setup_build_flags(
