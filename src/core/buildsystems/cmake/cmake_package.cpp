...
std::expected<void, std::string> CMakePackage::build(const std::filesystem::path& source_path) const {
    ...
    if (!has_cmake_file(actual_source_path)) {
        return std::unexpected("No CMakeLists.txt found in source directory: " + actual_source_path.string());
    }
...
std::expected<void, std::string> CMakePackage::configure_cmake(const std::filesystem::path& source_path) const {
    if (!command_executor_) {
    ...
    auto result = cppup::package::utils::execute_command(*command_executor_, configure_command, source_path);
    if (!result) {
...
std::expected<void, std::string> CMakePackage::build_cmake(const std::filesystem::path& source_path) const {
    if (!command_executor_) {
        return std::unexpected("No command executor available");
    }
    ...
    auto result = cppup::package::utils::execute_command(*command_executor_, build_command, source_path);
    if (!result) {
...
void CMakePackage::setup_build_flags(
...
std::string CMakePackage::get_cmake_configure_command(const std::filesystem::path& source_path) const {
