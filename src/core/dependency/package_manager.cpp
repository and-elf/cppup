...
std::expected<void, std::string> PackageManager::install_package(
...
std::expected<void, std::string> PackageManager::remove_package(
    const std::string& name, const std::string& version, const RemovalOptions& options) noexcept
{
  try
  {
    // Get package info
...
std::expected<std::vector<PackageInfo>, std::string> PackageManager::list_installed() const noexcept
{
  if (!database_)
  {
...
std::expected<PackageInfo, std::string> PackageManager::get_package_info(
...
std::expected<void, std::string> PackageManager::add_source(const PackageSource& source) noexcept
{
...
{
  return {cppup_registry(), vcpkg_registry(), conan_center()};
}
