...
public:
     ...
     */
    [[nodiscard]] std::expected<ResolutionResult, std::string>
    resolve(const std::vector<DependencyRequirement>& requirements) noexcept;
    
    /**
     * Check if a package version satisfies constraints
     */
    [[nodiscard]] bool satisfies_constraints(const std::string& package_name,
    ...
    find_best_version(const std::string& package_name,
                     const std::vector<VersionConstraint>& constraints) const noexcept;
    ...
    detect_cycles(const std::vector<DependencyRequirement>& requirements) const noexcept;
    
    /**
     * Generate dependency graph in DOT format for visualization
     */
    [[nodiscard]] std::expected<std::string, std::string>
    generate_dependency_graph(const ResolutionResult& result) const noexcept;
    
    /**
     * Update resolver configuration
     */
    void update_config(const ResolverConfig& config) { config_ = config; }
    
    /**
     * Get current configuration
     */
    [[nodiscard]] const ResolverConfig& get_config() const { return config_; }
...
private:
    ...
    resolve_recursive(const DependencyRequirement& requirement, 
     ...
     */
    [[nodiscard]] std::expected<std::string, std::string>
    resolve_version_conflict(const std::string& package_name,
                           const std::vector<VersionConstraint>& constraints) const noexcept;
    ...
    get_available_versions(const std::string& package_name) const noexcept;
...
namespace version_utils {
    ...
    filter_versions(const std::vector<std::string>& versions, 
