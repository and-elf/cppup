...
using ProgressCallback =
    std::function<void(const std::string& package, size_t downloaded, size_t total)>;

/**
 * Build progress callback
 */
using BuildCallback = std::function<void(const std::string& package, const std::string& stage,
...
class PackageManager
{
 public:
  ...
  [[nodiscard]] std::expected<void, std::string> install_package(
      const std::string& name, const std::string& version_constraint = "",
      const InstallOptions& options = {}) noexcept;

  /**
   * Install multiple packages
   */
  [[nodiscard]] std::expected<void, std::string> install_packages(
   ...
   */
  [[nodiscard]] std::expected<void, std::string> update_package(const std::string& name) noexcept;

  /**
   * Update all packages
   */
  [[nodiscard]] std::expected<void, std::string> update_all_packages() noexcept;

  /**
   * List installed packages
   */
  [[nodiscard]] std::expected<std::vector<PackageInfo>, std::string> list_installed()
      const noexcept;

  /**
   * Search for packages in registries
   */
  [[nodiscard]] std::expected<std::vector<RegistryEntry>, std::string> search_packages(
      const std::string& query) const noexcept;

  /**
   * Get package information
   */
  [[nodiscard]] std::expected<PackageInfo, std::string> get_package_info(
   ...
   */
  [[nodiscard]] std::expected<bool, std::string> verify_package(
      const std::string& name, const std::string& version) const noexcept;

  /**
   * Clean package cache
   */
  [[nodiscard]] std::expected<void, std::string> clean_cache() noexcept;

  /**
   * Add package source
   */
  [[nodiscard]] std::expected<void, std::string> add_source(const PackageSource& source) noexcept;

  /**
   * Remove package source
   */
  [[nodiscard]] std::expected<void, std::string> remove_source(const std::string& name) noexcept;
   ...
   */
  [[nodiscard]] std::expected<void, std::string> update_registry() noexcept;
  ...
  void set_build_callback(BuildCallback callback)
  {
 ...
 private:
   ...
   */
  [[nodiscard]] std::expected<std::filesystem::path, std::string> download_package(
      const std::string& name, const std::string& version, const PackageSource& source) noexcept;

  /**
   * Extract package archive
   */
  [[nodiscard]] std::expected<std::filesystem::path, std::string> extract_package(
      const std::filesystem::path& archive_path, const std::string& name,
      const std::string& version) noexcept;

  /**
   * Build package from source
   */
  [[nodiscard]] std::expected<void, std::string> build_package(
      const std::filesystem::path& source_dir, const std::string& name, const std::string& version,
      const InstallOptions& options) noexcept;

  /**
   * Install built package
   */
  [[nodiscard]] std::expected<void, std::string> install_built_package(
      const std::filesystem::path& build_dir, const PackageInfo& package,
      const InstallOptions& options) noexcept;

  /**
   * Calculate package checksum
   */
  [[nodiscard]] std::expected<std::string, std::string> calculate_checksum(
      const std::filesystem::path& package_dir) const noexcept;
   ...
   */
  [[nodiscard]] std::optional<std::string> get_installed_version(
   ...
   */
  void notify_build_progress(const std::string& package, const std::string& stage,
