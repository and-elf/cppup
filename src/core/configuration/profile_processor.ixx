export module cppup.configuration.profile_processor;
 ...
 */
export struct ProfileProcessingResult {
    bool success = false;
...
public:
     ...
     */
    [[nodiscard]] std::string get_effective_profile_name(
        const BuildConfiguration& config,
        const std::string& profile_name = ""
    ) const;
     ...
     */
    [[nodiscard]] const Profile* find_profile(
        const BuildConfiguration& config,
        const std::string& profile_name
    ) const;
     ...
     */
    [[nodiscard]] BuildConfiguration merge_profile(
        const BuildConfiguration& base_config,
...
private:
     ...
     */
    template<typename T>
    [[nodiscard]] std::vector<T> merge_vectors(
        const std::vector<T>& base,
        const std::vector<T>& additional
    ) const {
     ...
     * @return Merged flags
     */
    [[nodiscard]] std::vector<Flag> merge_flags(
        const std::vector<Flag>& base,
        const std::vector<Flag>& additional
    ) const {
    ...
    [[nodiscard]] std::vector<Definition> merge_definitions(
        const std::vector<Definition>& base,
        const std::vector<Definition>& additional
    ) const {
...
ProfileProcessingResult ProfileProcessor::process_profiles(
    const BuildConfiguration& config,
    const std::string& profile_name
) const {
...
std::string ProfileProcessor::get_effective_profile_name(
    const BuildConfiguration& config,
    const std::string& profile_name
) const {
...
const Profile* ProfileProcessor::find_profile(
    const BuildConfiguration& config,
    const std::string& profile_name
) const {
...
BuildConfiguration ProfileProcessor::merge_profile(
    const BuildConfiguration& base_config,
    const Profile& profile
) const {
...
bool ProfileProcessor::has_profile(
    const BuildConfiguration& config,
    const std::string& profile_name
) const {
