export module cppup.configuration.toolchain_resolver;
 ...
 */
export struct ResolvedToolchain {
    std::string name;
    std::string version;
 ...
 */
export struct ToolchainResolutionResult {
    bool success = false;
    ...
    [[nodiscard]] bool has_toolchain() const noexcept { return toolchain.has_value(); }
};

/**
 * Interface for toolchain information provider (to be implemented by CLI system)
 */
export class ToolchainInfoProvider {
public:
     ...
     */
    [[nodiscard]] virtual std::optional<ResolvedToolchain> get_toolchain_info(const std::string& name) const = 0;
...
public:
     ...
     */
    [[nodiscard]] std::optional<std::string> get_effective_toolchain_name(const BuildConfiguration& config) const;
    ...
    [[nodiscard]] BuildConfiguration apply_toolchain_settings(
        ...
        const ResolvedToolchain& resolved_toolchain
    ) const;
 ...
 */
export class MockToolchainInfoProvider : public ToolchainInfoProvider {
public:
    ...
    [[nodiscard]] std::optional<ResolvedToolchain> get_toolchain_info(const std::string& name) const override {
...
ToolchainResolutionResult ToolchainResolver::resolve_toolchain(const BuildConfiguration& config) const {
...
std::optional<std::string> ToolchainResolver::get_effective_toolchain_name(const BuildConfiguration& config) const {
...
BuildConfiguration ToolchainResolver::apply_toolchain_settings(
    ...
    config.include_paths.insert(
        config.include_paths.begin(),
