...
void test_find_profile()
{
  ...
  BuildConfiguration config{.profiles = {Profile{"debug"}, Profile{"release"}, Profile{"test"}}};
...
void test_has_profile()
{
  ...
  BuildConfiguration config{.profiles = {Profile{"debug"}, Profile{"release"}}};
...
void test_validate_profiles()
{
  ...
  BuildConfiguration valid_config{
      .profiles = {Profile{"debug"}, Profile{"release"}, Profile{"test"}}};

  auto error = processor.validate_profiles(valid_config);
  assert(error.empty());

  // Test duplicate profile names
  BuildConfiguration duplicate_config{.profiles = {
                                          Profile{"debug"}, Profile{"release"}, Profile{"debug"}
                                          // Duplicate
                                      }};
  ...
  BuildConfiguration empty_name_config{.profiles = {empty_profile}};
...
void test_merge_profile_packages()
{
  ...
  BuildConfiguration base_config{.packages = {Package{"boost", "1.82.0"}, Package{"fmt"}}};
...
void test_merge_profile_flags()
{
  ...
  BuildConfiguration base_config{.compile_flags = {Flag{"-Wall"}, Flag{"-Wextra"}},
  ...
  {
    if (flag.flag == "-pthread")
      has_pthread = true;
    else if (flag.flag == "-rdynamic")
      has_rdynamic = true;
  }
...
void test_merge_profile_definitions()
{
  ...
  BuildConfiguration base_config{
      .definitions = {Definition{"VERSION", "1.0.0"}, Definition{"FEATURE_A", "1"}}};
...
void test_merge_profile_include_paths()
{
  ...
  BuildConfiguration base_config{.include_paths = {"include/", "third_party/"}};
  ...
  assert(std::find(merged.include_paths.begin(), merged.include_paths.end(), "include/") !=
         merged.include_paths.end());
  assert(std::find(merged.include_paths.begin(), merged.include_paths.end(), "third_party/") !=
         merged.include_paths.end());
  assert(std::find(merged.include_paths.begin(), merged.include_paths.end(), "debug/include/") !=
         merged.include_paths.end());
...
void test_process_profiles_no_profiles()
{
  ...
  BuildConfiguration config{.packages = {Package{"boost"}}, .compile_flags = {Flag{"-Wall"}}};
...
void test_process_profiles_with_default()
{
  ...
  BuildConfiguration config{.packages      = {Package{"boost"}},
                            ...
                            .profiles      = {Profile{"debug"}, Profile{"release"}}};
...
void test_process_profiles_validation_error()
{
  ...
  BuildConfiguration config{.profiles = {
                                Profile{"debug"}, Profile{"debug"}  // Duplicate
                            }};
