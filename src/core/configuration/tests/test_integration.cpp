...
void test_complete_configuration_workflow()
{
  ...
  config.toolchain     = Toolchain{"gcc-13"};
  config.packages      = {Package{"fmt", "10.1.1"}, Package{"catch2"}};
  config.sources       = {"src/*.cpp"};
  config.compile_flags = warnings::extra();
  config.compile_flags.push_back(cpp_standard::cpp20());
  ...
  when_env(config, "BUILD_TYPE", "debug",
           [&]()
           {
             ...
             config.compile_flags.insert(config.compile_flags.end(), debug_flags.begin(),
                                         debug_flags.end());
             config.compile_flags.push_back(Flag{"-g"});
           });
  ...
  config.profiles = {debug_profile({Flag{"-fsanitize=address"}}),
                     release_profile({Flag{"-march=native"}}), test_profile("catch2")};

  // Custom build steps
  config.build_steps = {
      BuildStep("generate_version", []() { std::cout << "Generating version header...\n"; }),
      BuildStep("copy_assets", []() { std::cout << "Copying asset files...\n"; })
          .depends_on({"generate_version"})};
...
void test_library_api_completeness()
{
  ...
  platform::add_platform_packages(test_config, {Package{"win"}}, {Package{"linux"}},
                                  {Package{"mac"}});
  platform::add_platform_flags(test_config, {Flag{"-DWIN"}}, {Flag{"-DLINUX"}}, {Flag{"-DMAC"}});
...
void test_cross_platform_compatibility()
{
  ...
  platform::add_platform_packages(config, {Package{"windows_pkg"}}, {Package{"linux_pkg"}},
                                  {Package{"macos_pkg"}});

  platform::add_platform_flags(config, {Flag{"-DWINDOWS"}}, {Flag{"-DLINUX"}}, {Flag{"-DMACOS"}});
