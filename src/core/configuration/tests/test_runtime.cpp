...
void test_realistic_runtime_configuration()
{
  ...
  when_feature(config, "nonexistent", [&]() { packages.push_back("should_not_be_added"); });
  ...
  when_env(config, "OPTIMIZATION", "O2", [&]() { compile_flags.push_back("-O2"); });

  when_env(config, "TARGET", "development", [&]() { definitions.push_back("DEV_BUILD=1"); });

  when_env(config, "TARGET", "production", [&]() { definitions.push_back("PROD_BUILD=1"); });
  ...
  assert(std::find(definitions.begin(), definitions.end(), "HAVE_THREADING=1") !=
         definitions.end());
