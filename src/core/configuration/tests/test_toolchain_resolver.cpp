...
void setup_mock_toolchains(MockToolchainInfoProvider& provider)
{
  ...
  provider.add_toolchain(
      {.name                  = "msvc-2022",
       ...
       .linker_path           = "C:/Program Files/Microsoft Visual "
                                "Studio/2022/Community/VC/Tools/MSVC/14.37.32822/bin/Hostx64/x64/link.exe",
       .archiver_path         = "C:/Program Files/Microsoft Visual "
       ...
       .system_include_paths  = {"C:/Program Files/Microsoft Visual "
                                  "Studio/2022/Community/VC/Tools/MSVC/14.37.32822/include"},
       .system_library_paths  = {"C:/Program Files/Microsoft Visual "
...
void test_nonexistent_toolchain_resolution()
{
  ...
  BuildConfiguration config{.toolchain = Toolchain{"nonexistent-compiler"}};
...
void test_effective_toolchain_name()
{
  ...
  BuildConfiguration config_explicit{.toolchain = Toolchain{"clang-17"}};
...
void test_apply_toolchain_settings()
{
  ...
  assert(std::find_if(updated_config.compile_flags.begin(), updated_config.compile_flags.end(),
                      [](const Flag& f)
                      { return f.flag == "-Wall"; }) != updated_config.compile_flags.end());
  assert(std::find_if(updated_config.compile_flags.begin(), updated_config.compile_flags.end(),
                      [](const Flag& f)
                      { return f.flag == "-O2"; }) != updated_config.compile_flags.end());
  ...
  assert(std::find_if(updated_config.link_flags.begin(), updated_config.link_flags.end(),
                      [](const Flag& f)
                      { return f.flag == "-pthread"; }) != updated_config.link_flags.end());
...
void test_target_platform_inference()
{
  ...
  provider->add_toolchain({.name                  = "x86_64-linux-gnu-gcc",
                           .version               = "11.0.0",
  ...
  provider->add_toolchain({.name                  = "aarch64-linux-gnu-gcc",
                           .version               = "11.0.0",
