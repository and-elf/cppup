...
{
  // Add boost package
  provider.add_package({.name          = "boost",
                        .version       = "1.82.0",
  ...
  provider.add_package({.name          = "spdlog",
                        .version       = "1.12.0",
  ...
  // Add catch2 package
  provider.add_package({.name          = "catch2",
                        .version       = "3.4.0",
                        .compile_flags = {},
                        .link_flags    = {},
...
void test_simple_package_resolution()
{
  ...
  BuildConfiguration config{.packages = {Package{"boost", "1.82.0"}}};
  ...
  assert(!result.all_compile_flags.empty());
  assert(std::find(result.all_compile_flags.begin(), result.all_compile_flags.end(),
                   "-DBOOST_ALL_NO_LIB") != result.all_compile_flags.end());
  assert(!result.all_include_paths.empty());
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/boost") != result.all_include_paths.end());
  assert(!result.all_libraries.empty());
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "boost_system") !=
         result.all_libraries.end());
...
void test_multiple_package_resolution()
{
  ...
  BuildConfiguration config{.packages = {Package{"boost", "1.82.0"}, Package{"fmt", "10.1.1"},
  ...
  assert(std::find(result.all_compile_flags.begin(), result.all_compile_flags.end(),
                   "-DBOOST_ALL_NO_LIB") != result.all_compile_flags.end());
  assert(std::find(result.all_compile_flags.begin(), result.all_compile_flags.end(),
                   "-DFMT_HEADER_ONLY") != result.all_compile_flags.end());

  // Check aggregated include paths
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/boost") != result.all_include_paths.end());
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/fmt") != result.all_include_paths.end());
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/catch2") != result.all_include_paths.end());

  // Check aggregated libraries
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "boost_system") !=
         result.all_libraries.end());
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "fmt") !=
         result.all_libraries.end());
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "Catch2Main") !=
         result.all_libraries.end());
...
void test_transitive_dependency_resolution()
{
  ...
  BuildConfiguration config{.packages = {Package{"spdlog", "1.12.0"}}};
  ...
  assert(std::find(result.all_compile_flags.begin(), result.all_compile_flags.end(),
                   "-DSPDLOG_COMPILED_LIB") != result.all_compile_flags.end());
  assert(std::find(result.all_compile_flags.begin(), result.all_compile_flags.end(),
                   "-DFMT_HEADER_ONLY") != result.all_compile_flags.end());

  // Check that include paths from both packages are included
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/spdlog") != result.all_include_paths.end());
  assert(std::find(result.all_include_paths.begin(), result.all_include_paths.end(),
                   "/usr/include/fmt") != result.all_include_paths.end());

  // Check that libraries from both packages are included
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "spdlog") !=
         result.all_libraries.end());
  assert(std::find(result.all_libraries.begin(), result.all_libraries.end(), "fmt") !=
         result.all_libraries.end());
...
void test_nonexistent_package_resolution()
{
  ...
  BuildConfiguration config{.packages = {Package{"nonexistent"}}};
...
void test_duplicate_package_handling()
{
  ...
  BuildConfiguration config{.packages = {
                                Package{"fmt", "10.1.1"}, Package{"spdlog", "1.12.0"}
                                // This depends on fmt 10.1.1
                            }};
...
void test_resolver_without_provider()
{
  ...
  BuildConfiguration config{.packages = {Package{"boost"}}};
