...
void test_validate_build_steps()
{
  ...
  std::vector<BuildStep> valid_steps = {BuildStep{"step1", []() {}},
  ...
  std::vector<BuildStep> duplicate_steps = {
      BuildStep{"step1", []() {}}, BuildStep{"step1", []() {}}  // Duplicate name
  };
  ...
  std::vector<BuildStep> circular_steps = {BuildStep{"step1", []() {}}.depends_on({"step2"}),
                                           BuildStep{"step2", []() {}}.depends_on({"step1"})};
...
void test_create_execution_order()
{
  ...
  std::vector<BuildStep> steps = {BuildStep{"step3", []() {}}.depends_on({"step1", "step2"}),
                                  BuildStep{"step1", []() {}},
                                  BuildStep{"step2", []() {}}.depends_on({"step1"})};
  ...
  std::vector<BuildStep> no_deps = {BuildStep{"step1", []() {}}, BuildStep{"step2", []() {}},
                                    BuildStep{"step3", []() {}}};
...
void test_execute_single_step()
{
  ...
  bool      executed = false;
  BuildStep success_step("test_step", [&executed]() { executed = true; });
  ...
  // Test failing step
  BuildStep failing_step("failing_step", []() { throw std::runtime_error("Test error"); });
...
void test_dependencies_satisfied()
{
  ...
  std::vector<BuildStep> steps = {BuildStep{"step1", []() {}},
                                  BuildStep{"step2", []() {}}.depends_on({"step1"}),
                                  BuildStep{"step3", []() {}}.depends_on({"step1", "step2"})};
...
void test_execute_build_steps_with_failure()
{
  ...
  BuildConfiguration config{
      .build_steps = {
          ...
          BuildStep{"step2", []() { throw std::runtime_error("Step 2 failed"); }}.depends_on(
              {"step1"}),
          BuildStep{"step3", [&]() { step3_executed = true; }}.depends_on({"step2"})}};
...
void test_status_callback()
{
  ...
  BuildConfiguration config{
      .build_steps = {BuildStep{
          "test_step", []() { std::this_thread::sleep_for(std::chrono::milliseconds(10)); }}}};
