...
void test_platform_detection_reexport()
{
  ...
  int executed_count =
      (windows_executed ? 1 : 0) + (linux_executed ? 1 : 0) + (macos_executed ? 1 : 0);
...
void test_runtime_queries_reexport()
{
  ...
  when_feature(config, "test_feature", [&]() { feature_executed = true; });
  assert(feature_executed);

  bool env_executed = false;
  when_env(config, "TEST_VAR", "test_value", [&]() { env_executed = true; });
  assert(env_executed);
...
void test_release_profile_helper()
{
  ...
  for (const auto& flag : profile.compile_flags)
  {
    if (flag.flag == "-O3")
      has_o3 = true;
    else if (flag.flag == "-DNDEBUG")
      has_ndebug = true;
    else if (flag.flag == "-flto")
      has_lto = true;
  }
...
void test_platform_helpers()
{
  ...
  platform::add_platform_packages(config, {Package{"windows-only"}},  // Windows packages
  ...
  platform::add_platform_flags(config, {Flag{"-DWINDOWS"}},  // Windows flags
