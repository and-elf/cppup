...
void test_platform_specific_configuration()
{
  ...
  {
    assert(std::find(compile_flags.begin(), compile_flags.end(), "-Wall") != compile_flags.end());
    assert(std::find(link_flags.begin(), link_flags.end(), "-framework Foundation") !=
           link_flags.end());
  ...
  if (TARGET_ARCH != "unknown")
  {
    if (is_x86_64())
    {
      assert(std::find(compile_flags.begin(), compile_flags.end(), "-march=native") !=
             compile_flags.end());
    ...
    if (is_arm64())
    {
      assert(std::find(compile_flags.begin(), compile_flags.end(), "-mcpu=native") !=
             compile_flags.end());
