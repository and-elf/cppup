...
int main()
{
  ...
  if (init_result)
  {
    ...
    if (std::filesystem::exists(project_dir))
    {
      ...
      // Check for key files
      std::vector<std::string> expected_files = {"build.cpp",    "src/main.cpp", ".clang-format",
                                                 ".gitignore",   "README.md",    "setup_env.sh",
                                                 "setup_env.bat"};
      ...
      std::vector<std::string> expected_dirs = {".cppup", ".cppup/bin", ".cppup/packages",
                                                ".cppup/toolchains"};
