...
std::string_view map_source_type(conf::SourceType type) noexcept
{
  switch (type)
  {
    case conf::SourceType::DIRECTORY:
      return kSourceDirectory;
...
void write_entry(std::ostringstream& output, const Entry& entry)
{
  ...
  output << "source = " << quote(entry.source) << '\n';
                                                                  ...
                                                                  const KeyValueLine& line)
{
  ...
  // Free-form: built-in kinds match `kSource*`; unknown kinds round-trip
  // verbatim so plugin-defined sources survive lock/sync cycles.
  entry.source = std::move(*unquoted);
  return {};
}
...
Entry entry_from_info(const conf::PackageInfo& info)
{
  ...
  entry.source       = std::string(map_source_type(info.source_type));
  ...
  if (entry.source == kSourceDirectory && entry.url.empty())
  {
