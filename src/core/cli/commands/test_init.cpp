...
TEST(Init, MinimalEmitsBaseFilesOnly)
{
  ...
  const auto proj = root / "myapp";
  EXPECT_TRUE(fs::exists(proj / "build.cpp"));
  EXPECT_TRUE(fs::exists(proj / "src" / "main.cpp"));
  EXPECT_TRUE(fs::exists(proj / "tests" / "test_main.cpp"));
  EXPECT_TRUE(fs::exists(proj / ".clang-format"));
  EXPECT_TRUE(fs::exists(proj / ".gitignore"));
  EXPECT_TRUE(fs::exists(proj / "README.md"));

  EXPECT_FALSE(fs::exists(proj / ".vscode"));
  EXPECT_FALSE(fs::exists(proj / ".devcontainer"));
  EXPECT_FALSE(fs::exists(proj / "Dockerfile"));
  EXPECT_FALSE(fs::exists(proj / ".gitlab-ci.yml"));
...
TEST(Init, TestTemplateUsesGoogleTest)
{
  ...
  const auto test_main = slurp(root / "g" / "tests" / "test_main.cpp");
  ...
  const auto build_cpp = slurp(root / "g" / "build.cpp");
...
TEST(Init, ProjectNamePlaceholderSubstituted)
{
  ...
  const auto build_cpp = slurp(root / "acme_tool" / "build.cpp");
  const auto readme    = slurp(root / "acme_tool" / "README.md");
...
TEST(Init, WithDevcontainerEmitsDevcontainerJson)
{
  ...
  EXPECT_TRUE(fs::exists(root / "p" / ".devcontainer" / "devcontainer.json"));
...
TEST(Init, WithDockerEmitsDockerfile)
{
  ...
  EXPECT_TRUE(fs::exists(root / "p" / "Dockerfile"));
  EXPECT_NE(slurp(root / "p" / "Dockerfile").find("debian:trixie-slim"), std::string::npos)
...
TEST(Init, WithGitlabCiEmitsGitlabCiYml)
{
  ...
  EXPECT_TRUE(fs::exists(root / "p" / ".gitlab-ci.yml"));
...
TEST(Init, FullEnablesAllFeatures)
{
  ...
  const auto proj = root / "full";
  EXPECT_TRUE(fs::exists(proj / ".vscode" / "tasks.json"));
  EXPECT_TRUE(fs::exists(proj / ".devcontainer" / "devcontainer.json"));
  EXPECT_TRUE(fs::exists(proj / "Dockerfile"));
  EXPECT_TRUE(fs::exists(proj / ".gitlab-ci.yml"));

  fs::remove_all(root);
}

TEST(Init, RejectsExistingProjectDirectory)
{
...
TEST(Init, RejectsEmptyProjectName)
{
  auto       root = make_tmp_root("empty");
  const auto ctx  = make_ctx(root);

  EXPECT_FALSE(executeInit("", std::nullopt, InitOptions{}, ctx).has_value());
...
TEST(Init, GitignoreDoesNotExcludeShippedVscodeDir)
{
  ...
  const auto gi = slurp(root / "p" / ".gitignore");
