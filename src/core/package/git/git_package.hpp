...
/**
 * Package implementation for Git repositories
 */
class GitPackage
{
 public:
  explicit GitPackage(cppup::configuration::PackageInfo info);
 ...
 private:
  ...
  std::expected<std::filesystem::path, std::string> clone_repository() const;
  std::expected<void, std::string> checkout_commit(const std::filesystem::path& repo_path) const;
};
